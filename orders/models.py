...
class Order(models.Model):
    ...
    requires_delivery = models.BooleanField(
        default=False, blank=True, verbose_name="Оплата при получении"
    )
    ...
    def __str__(self):
        return f"Заказ № {self.id} | Покупатель {self.user.first_name} {self.user.last_name}"


class OrderItem(models.Model):
    order = models.ForeignKey(to=Order, on_delete=models.CASCADE, verbose_name="Заказы")
